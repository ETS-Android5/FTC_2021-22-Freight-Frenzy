...
@Autonomous(name="Auto_Spin_Park_Block", group="")
public class Auto_Spin_Park_Block extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        //Drive to park
        timmer.reset();
        while(timmer.milliseconds() < 2200) {
            correction = checkDirection_90();
            correction=0;
            LB_Drive.setPower((power * 1) + correction);
            RB_Drive.setPower((power * 1) - correction);
        ...
        // straiff to wobble
        timmer.reset();
        while(timmer.milliseconds() < 3500) {
            correction = checkDirection_90();
            correction=0;
            LB_Drive.setPower((power * -1) + correction);
            RB_Drive.setPower((power * 1) - correction);
        ...
        LF_Drive.setPower(0);
        RF_Drive.setPower(0);
        // Lift elevator to level
        timmer.reset();
        while(timmer.milliseconds() < 3800) {
        ...
        // straiff to wall
        timmer.reset();
        while(timmer.milliseconds() < 4500) {
            correction = checkDirection_90();
            correction=0;
            LB_Drive.setPower((power * 1) + correction);
            RB_Drive.setPower((power * -1) - correction);
